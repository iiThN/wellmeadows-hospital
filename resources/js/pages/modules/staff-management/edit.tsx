...
export default function StaffEdit({ staff }: { staff: Staff }) {
    const { data, setData } = useForm({
        ...
        sex:            staff.sex,
        nin:            staff.nin,
        current_salary: String(staff.current_salary),
        salary_scale:   staff.salary_scale,
        pay_type:       staff.pay_type,
        hours_per_week: String(staff.hours_per_week),
    ...
    async function submit(e: React.FormEvent) {
        ...
        setFormErrors({});
        try {
            await axios.put(`/modules/staff-management/${staff.staff_number}`, {
                ...data,
                ...
                <Section title="Employment Details">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <Field label="Salary Scale" error={err('salary_scale')}>
                            <input value={data.salary_scale} onChange={e => setData('salary_scale', e.target.value)} className={inp} />
                        </Field>
                    </div>
