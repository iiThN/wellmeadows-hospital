...
import { pharmaceuticalSupplies, patientMedications, patients } from "../data/mockData"
...
function Medication() {
  ...
  const filteredDrugs = pharmaceuticalSupplies.filter(d =>
    d.drug_name.toLowerCase().includes(search.toLowerCase()) ||
    d.description.toLowerCase().includes(search.toLowerCase())
  )

  const filteredRx = patientMedications.filter(m => {
    const p = patients.find(pt => pt.patient_no === m.patient_no)
    const d = pharmaceuticalSupplies.find(dr => dr.drug_no === m.drug_no)
    ...
    <div className="page">
      <div className="page__head">
        <div>
          ...
          <div className="page__subtitle">{pharmaceuticalSupplies.length} drugs · {patientMedications.length} prescriptions</div>
        </div>
      </div>
      ...
      <div className="stats" style={{ gridTemplateColumns: "repeat(4, 1fr)" }}>
        <div className="stat stat--blue"><div className="stat__label">Total drugs</div><div className="stat__value">{pharmaceuticalSupplies.length}</div></div>
        <div className="stat stat--red"><div className="stat__label">Low stock</div><div className="stat__value">{lowStock.length}</div></div>
        <div className="stat stat--teal"><div className="stat__label">Prescriptions</div><div className="stat__value">{patientMedications.length}</div></div>
        <div className="stat stat--purple">
          <div className="stat__label">Patients medicated</div>
          <div className="stat__value">{[...new Set(patientMedications.map(m => m.patient_no))].length}</div>
        </div>
      </div>
              ...
              <tbody>
                {filteredDrugs.map(d => (
                  <tr key={d.drug_no}>
                    ...
                    <td>£{d.cost_per_unit.toFixed(2)}</td>
                  </tr>
                ))}
